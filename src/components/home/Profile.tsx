...
export default function Profile({ author, social, features, researchInterests }: ProfileProps) {
    ...
    return (
        ...
        >
            {/* Profile Image */}
            <div className="w-full aspect-[3/4] mb-5 rounded-xl overflow-hidden">
                <Image
                    src={author.avatar}
                    alt={author.name}
                    width={256}
            ...
            <div className="text-left mb-6">
                <h1 className="text-2xl font-bold text-foreground mb-1" style={{ fontFamily: "'p22-mackinac-pro', serif" }}>
                    {author.name}
                    {(() => {
                        const parts = author.title.split('·');
                        const pronouns = parts.length > 1 ? parts[1].trim() : null;
                        return pronouns ? (
                            <span className="text-sm font-normal text-neutral-400 ml-2">({pronouns})</span>
                        ) : null;
                    })()}
                </h1>
                {(() => {
                    const parts = author.title.split('·');
                    return (
                        <p className="text-lg text-accent font-medium mb-0.5">{parts[0].trim()}</p>
                    );
                })()}
                {author.institution.split('·').map((part, i) => (
                    <p key={i} className="text-neutral-600 text-sm mb-0.5">
                        {part.trim()}
                    </p>
                ))}
            </div>
            ...
            {/* Contact Links */}
            <div className="flex flex-wrap justify-start gap-3 sm:gap-4 mb-6 relative">
